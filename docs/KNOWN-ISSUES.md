...
- Full DGX OS and Omarchy default application parity is incomplete. The port
  ...
  `scripts/audit-base-packages.py`. On the test Spark 124 of the 147 base
  packages are installed and every package the Arch ARM repos offer is
  present. The 23 unavailable ones are tiered: 14 are Omarchy recipes that
  declare aarch64 and still need building (ttfx is done), 4 are external
  packages needing their own recipes, 3 are x86-only recipes and 2 do not
  apply. The installer's post-install and first-user provisioning steps are
  still reproduced by hand.
